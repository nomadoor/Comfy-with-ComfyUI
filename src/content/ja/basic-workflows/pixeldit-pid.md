...
画像をそのまま 1 ピクセルずつ計算するのは大変なので、一度 latent という小さな表現に圧縮して、その中で生成します。
これによって計算量を減らしつつ、形や色、構図のような特徴を扱いやすくしていました。

ただし、VAE による圧縮と復元は完全ではありません。
細かい文字、模様、エッジ、質感のような情報は、ここで少し崩れてしまうことがあります。

PixelDiT は、そこを避けるために、画像をピクセル空間のまま扱います。
latent を介さないので、VAE による復元劣化は仕組み的に起きません。

もちろん、計算量を下げるための latent だったんじゃないの？という疑問は残ります。

そこで PixelDiT では、画像全体をそのまま細かく見るのではなく、パッチ単位で大まかな構図や意味を扱い、必要な部分をピクセル側で細かく詰める、という構造になっています。
ざっくり言えば、latent に逃がしていた計算量の問題を、パッチ分割と二段構えの処理で受け止めているわけです。
