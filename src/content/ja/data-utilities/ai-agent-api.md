...
AI に ComfyUI を使わせる、と聞くと、AI がブラウザを開いてノード画面を操作するようなものを想像するかもしれません。

もちろん、それに近いことも不可能ではありません。  
ただ、最初に目指す方法としてはあまりおすすめしません。

ノード画面は、人間が workflow を作るための画面です。  
AI にマウス操作をさせたり、ノードをつないだりさせるよりも、すでに動く workflow を API 経由で実行させるほうがずっと安定します。

AI に ComfyUI の画面そのものを自由操作させるのではなく、**ComfyUI で作った workflow を、AI の道具として使わせる** という考え方です。

---

## この記事で扱うのは、動くworkflowをAPI経由で使わせる方法

AI に ComfyUI を使わせる、と言うと、AI が workflow をゼロから作るところまで想像するかもしれません。

しかしこの記事では、そこまでは扱いません。
...
つまり、AI に ComfyUI 全体を自由に使わせるというより、**ComfyUI で作ったひとつの機能を AI に渡す** という感じです。
...
## workflowを作るところから任せるのは別の話

AI に workflow をゼロから作らせる方法も考えられます。

たとえば「Flux で画像編集する workflow を作って」「この画像からマスクを作って合成する workflow を組んで」と頼み、AI がノード構成を考えて JSON を作る、という方向です。

ただし、これは今回扱う API 経由の実行とは別の問題です。  
ComfyUI の workflow は、その人の環境に入っている custom node やモデルファイル、ComfyUI 本体のバージョンに強く依存します。

そのため、AI が一般的な知識だけで workflow を作っても、自分の環境では動かないことがあります。

この話は重要ですが、この記事では深入りしません。  
ここではまず、すでに動く workflow を AI に使わせる方法に絞ります。
