...
- The Notes sidebar panel renders differently from other sections:
  ...
  - Updated / Views segment control,
  - flat article title list with no category headings or indentation.
- Add `/notes/find/` for local note discovery. It lists all Notes as cards, supports text filtering by title / summary / tags, exposes tag pills that write into the local search field, and provides Updated / Views sorting. Views falls back to Updated until view-count data exists.
