...
4. 1件の動作確認結果は `gpu`、`ram`、`time` と任意の `tags`、`samplers` を持つ。複数結果を登録できる。`samplers` の入力はComfyUI表示どおりの `s/it` または `it/s` を受け付けるが、popupでは比較しやすいよう `s/it` に統一する。`it/s` は逆数へ変換し、有効数字2桁で表示する。複数登録する場合は各Samplerを識別する短い `name` を必須とする。
...
10. popup右側は上段にWorkflow全体の `Total`、下段に任意のSampler速度を置き、左側のGPU・RAM行とそれぞれ中央を揃える。測定条件は入れ子のtooltipにせず、popup最下部へ「モデルUnload・Cache消去後の実測値」と短く常時表示する。注記の区切り線はpopupの内幅全体に引く。

## Consequences

- 記事作者が追加する概念は `level`、`gpu`、`ram`、`time`、任意の `tags` と `samplers` に限られる。
