...
- On desktop, the document/body is the native scroll root. The central reader surface is part of normal document flow, with a sticky non-interactive top cap that preserves the upper rounded edge below the fixed header. Wheel, keyboard, and middle-button scrolling use browser-native document behavior instead of JavaScript proxying.
...
- On desktop, `.app-shell__content-top` keeps the upper reader radius visible below the header while the document scrolls. `.app-shell__content-scroll` is a structural padding and width wrapper, not an independent page scroller.
