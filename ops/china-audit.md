...
## 7. Phased Rollout Plan (中国本土を視野に入れた段階導入)

### 7.1 方針
- 中国語ユーザー（中国本土以外）にデザイン劣化を強制しない
- ユーザーが手動で heavy/lite を切り替えなくても良い設計
- まずは「落ちたら段階的に切り替える」自動フェイルセーフを優先

### 7.2 段階導入（推奨）
- Phase A: 翻訳 + SEO正規化のみ
  - 影響最小、日英に影響なし
- Phase B: 自動フェイルセーフ
  - 外部フォント/埋め込みが失敗した場合のみ軽量化
  - 例: フォントCSSの fetch 失敗/タイムアウトで system stack に切替
  - 例: Gyazo動画がロード不能なら画像/リンク表示に退避
- Phase C: 中国本土向け最適化（必要に応じて）
  - エッジ/配信側で地理判定（例: CDNの地域ヘッダ）→ zh-lite を自動適用
  - 本土判定できない場合は「失敗検知 → 段階切替」を継続

### 7.3 地理判定について（注意）
- ブラウザの位置情報APIは許諾が必要でUXが悪い
- 可能なら配信側（CDN/Workers）で地域判定し、レスポンス側でクラス/フラグを付与
- 実装コストが高い場合は「フェイルセーフのみ」で十分に前進可能
