...
- **理由を後ろに投げる倒置を使わない**。「`true` に。あとで全部つなげるので」のような形にしない。普通の語順で書く。
- **「〜からです。」で理由を締めない**。説明口調になり、作文のように読める。1 文にまとめるか 2 文に分ける。
- ひと続きの補足を文ごとに別段落へ分けない。改行だけで十分な場合は、行末の半角スペース 2 個による Markdown の強制改行を使い、段落間の余白でページをむやみに縦へ伸ばさない。
...
- 通常の箇条書きは **ハイフン `-`** を使う。
- **項目を並べる箇条書きは言い切り**（「〜する」）。です・ますにしない。
  - 例: `- \`Simple\`：指定した回数だけ繰り返す`
  - ただし画像の下に置くキャプション文は、です・ます のままでよい。
- **1 項目だけの箇条書きは作らない**。それは箇条書きではなく地の文。
