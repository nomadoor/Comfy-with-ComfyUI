...
* 📝Notes
  ...
  * `noteTags` は処理上も表示上もフラットなタグとして扱う。ただし著者向けの付与ルールでは tier1 / tier2 を分ける。tier1 は `faq`、`troubleshoot`、`custom-nodes`、`concept`、`experiment` のようなカテゴリ寄りの主分類。tier2 は `install`、`vram`、`error`、モデル名・ツール名・補助トピックのような副分類。各Noteは原則としてtier1を1件以上持ち、tier2は必要な分だけ追加する
  * Note finder 上部のタグボタンは全 `noteTags` の一覧ではなく、`src/_data/noteFinder.json` で明示したtier1タグのうち、現在のNotesに実際に使われているものだけを表示する
