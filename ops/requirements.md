...
- Lightbox must support Esc close, continuous zoom from 100% to at least 500%, constrained drag/pinch pan, and arrow keys or buttons for navigation. After raw image load, raise the per-image maximum above 500% when needed to reach the raw image's intrinsic dimensions from the current fitted size; this keeps small-screen inspection from ending at a smaller effective image size than desktop. At 100%, fit the image between the previous/next buttons without covering them; clicking the zoom-in cursor image must then zoom to 200% and change the cursor to a grab hand for panning. Zoom also works through flat, borderless controls at the bottom right, keyboard +/- keys, mouse wheel, and touch pinch. Place the localized operation hint beside those controls and keep its wording unchanged across zoom states so both zoom and pan operations are always visible. Keep a clearly visible close `×` at the top right. The browser viewport is the pan canvas after zooming: enlarged pixels may pass behind the navigation controls and are clipped only at the viewport edge rather than the fitted image rectangle. Controls must overlay the canvas without reserving a bottom toolbar, and the hint must be hidden for video. Clicking outside the fitted image resets an enlarged image before closing the lightbox on the next click.
